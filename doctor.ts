...
const RUNTIME = (process.env.BIKLABS_RUNTIME_TYPE ?? process.env.BIK_RUNTIME_TYPE ?? "claude_code") as RuntimeType;
...
export async function main(): Promise<void> {
  let errors = 0;
  log("Running biklabs-agent doctor...");

  if (!AGENT_ID) {
    fail("Agent ID", "missing BIKLABS_AGENT_ID/AGENT_ID");
    errors += 1;
  } else {
  ...
  if (!AGENT_TOKEN) {
    fail("Agent token", "missing BIKLABS_AGENT_TOKEN/AGENT_MCP_TOKEN");
    errors += 1;
  } else {
