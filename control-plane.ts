...
export function resolveControlPlaneClient(): ControlPlaneClient | null {
  const timeoutMs = parseInt(process.env.BIKLABS_CONTROL_HTTP_TIMEOUT_MS ?? process.env.BIK_CONTROL_HTTP_TIMEOUT_MS ?? "10000", 10);
  ...
  const cpUrl = process.env.BIKLABS_CONTROL_PLANE_URL ?? process.env.BIK_CONTROL_PLANE_URL ?? "";
  const cpToken = process.env.BIKLABS_CONTROL_PLANE_TOKEN ?? process.env.BIK_CONTROL_PLANE_TOKEN ?? "";
  if (cpUrl && cpToken) {
...
export function buildSessionIdentity(agentId: string, runtimeType: string): SessionIdentity {
  return {
    sessionId: process.env.BIKLABS_AGENT_SESSION_ID ?? process.env.BIK_AGENT_SESSION_ID ?? randomUUID(),
    agentId,
    label: process.env.BIKLABS_AGENT_SESSION_LABEL ?? process.env.BIK_AGENT_SESSION_LABEL ?? `${agentId}-terminal`,
    host: process.env.BIKLABS_AGENT_SESSION_HOST ?? process.env.BIK_AGENT_SESSION_HOST ?? process.env.HOSTNAME ?? "local-terminal",
    runtimeType,
  };
}
