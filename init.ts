...
const DEFAULT_MCP_URL = process.env.BIKLABS_MCP_URL ?? process.env.BIK_MCP_URL ?? process.env.MCP_SERVER_URL ?? "https://devapi.biklabs.ai/v1/mcp";
const DEFAULT_CONFIG_DIR = process.env.BIKLABS_AGENT_CONFIG_DIR ?? process.env.BIK_AGENT_CONFIG_DIR ?? join(homedir(), ".biklabs-agent");
...
export async function main(): Promise<void> {
  ...
  try {
    ...
    process.stdout.write(`export BIKLABS_AGENT_ID="${agentId}"\n`);
    process.stdout.write(`export BIKLABS_AGENT_TOKEN="${token.trim()}"\n`);
    process.stdout.write(`export BIKLABS_MCP_URL="${mcpUrl}"\n`);
    process.stdout.write(`export BIKLABS_RUNTIME_TYPE="${runtimeType}"\n`);
    process.stdout.write(`export BIKLABS_WORK_DIR="${workDir}"\n`);
  } finally {
    rl.close();
  }
}
